...
## Create external table in Unity Catalog using three-level namespace
def create_uc_table(spark, catalog, schema, table, location):
  # Create table if not exists in Unity Catalog
  spark.sql(f"CREATE TABLE IF NOT EXISTS {catalog}.{schema}.{table} USING DELTA LOCATION '{location}'")
