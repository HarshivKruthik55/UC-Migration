...
tgtPath = settings[Environment]['GoldMountPath'] + "/source/sales/agg/pos_daily_agg_hhn"
goldTestResultsPath = settings[Environment]['GoldMountPath'] + "/source/sales/test_results/unit_testresults/pos_daily_agg_hhn"
...
tgtDF.createOrReplaceTempView("gold_pos_daily_agg_hhn")
...
expectedTrgColumns = ["fiscal_date", "hhn", "location_hk", "channel", "transaction_count", "sales", "scan", "units", "items", "dl_load_dt"]
...
#verify duplicate records in pos_daily_agg_hhn table gold layer
dupCntDf = spark.sql("""Select 'Gold_Pos_Daily_Agg_Hhn_Dup_Check' As Test_Case_Name, Case When Count(*) = 0 Then 'Pass' Else 'Fail' End As Result 
                         From 
                         (Select fiscal_date, hhn, location_hk, channel, Count(*) As Cnt 
                           From gold_pos_daily_agg_hhn
                          Group By fiscal_date, hhn, location_hk, channel Having Count(*) > 1)""")
dupCntDf.show()
...
ScanColNullDf = spark.sql("""Select 'Gold_Pos_Daily_Agg_Hhn_Scan_Null_Check' As Test_Case_Name, Case When Count(*) = 0 Then 'Pass' Else 'Fail' End As Result 
                         From gold_pos_daily_agg_hhn
                         Where scan is NULL""")
ScanColNullDf.show()
...
# DBTITLE 1,Household count check
...
custDF = spark.read.format("delta").load(cust_dim).select("customer_hk",F.col("current_household_id").alias("hhn"))
...
srcDF = retailSalesDF.alias("r").join(locationDF.alias("l"), F.col("r.location_hk") == F.col("l.location_hk"), "inner")\
                                               ...
                                               .select("c.hhn").distinct()
srcCnt = srcDF.count()
print(f"Source distinct cards count: {srcCnt}")

targetDF = tgtDF.select("hhn").distinct()
...
testCaseName = "household count Check"
executionDate = datetime.now(timezone('US/Pacific')).strftime('%Y-%m-%d %H:%M:%S') 
...
testCaseFailureTitle="Unit Test validations of Gold Layer pos_daily_agg_hhn table failed"
notebookName = "test_ut_z3_z3_pos_daily_agg_hhn_full_refresh"
...
try:
  if testCaseResult1 == 'Pass' and testCaseResult2 == 'Pass' and testCaseResult3 == 'Pass' and testCaseResult4 == 'Pass':
    print('Unit test validations of gold pos_daily_agg_hhn table succeeded')
  else:
    ...
    raise Exception ('Unit test validations of gold pos_daily_agg_hhn table failed')
finally:
